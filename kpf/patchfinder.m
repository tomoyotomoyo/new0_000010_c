...
int grab_kernelcache(void)
{
    ...
    if(is_kernelcache_valid(kc_copydst_path) == false) {
        printf("[%s:%d] Invalid kernelcache data :(]n", __FUNCTION__, __LINE__);
...
int init_xpf(void) {
    ...
    if (xpf_start_with_kernel_path(kernelcache_path.fileSystemRepresentation) == 0) {
        ...
        // === Resolve _msgbufp and _msgbufsize symbols ===
        extern uint64_t g_msgbufp_addr;
        extern uint64_t g_msgbuf_size;
        if (gXPF.kernel) {
            __block uint64_t msgbufp_vmaddr = 0;
            __block uint64_t msgbufsize_vmaddr = 0;
            macho_enumerate_symbols(gXPF.kernel, ^(const char *name, uint8_t type, uint64_t vmaddr, bool *stop) {
                if (strcmp(name, "_msgbufp") == 0) {
                    msgbufp_vmaddr = vmaddr;
                } else if (strcmp(name, "_msgbufsize") == 0) {
                    msgbufsize_vmaddr = vmaddr;
                }
            });
            g_msgbufp_addr = msgbufp_vmaddr;
            g_msgbuf_size = msgbufsize_vmaddr;
            printf("XPF symbols: _msgbufp=0x%llx _msgbufsize=0x%llx\n", msgbufp_vmaddr, msgbufsize_vmaddr);
        }
